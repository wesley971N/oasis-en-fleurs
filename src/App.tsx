...
const CATEGORIES = ['Tout', 'Phytembryothérapie', 'Huiles Essentielles', 'Hydrolats', 'Synergies', 'Tisanes & Plantes', 'Baumes', 'Savons', 'Miellerie', 'Laine']
...
La glycérine végétale, extraite du colza (teneur en huile de 50 à 68 %), extrait : huiles essentielles (phénols…), flavonoïdes liposolubles, vitamines liposolubles, certains acides.`,
  ...
  'Savons': `Savon artisanal Mille-Bulles, par Les Naturels de la Source. Nos savons reprennent les formulations de la savonnerie artisanale « Mille-Bulles », dont Agnès a racheté les recettes, désormais fabriqués à la ferme.

Ils sont réalisés selon le procédé traditionnel de saponification à froid, qui préserve la glycérine naturelle ainsi que les propriétés des huiles végétales. Enrichis de macérâts de plantes médicinales, d'huiles essentielles et de beurres végétaux, ils nourrissent, apaisent et protègent la peau en douceur. Sans détergents ni conservateurs de synthèse, chaque pain est façonné et découpé à la main.`,
            ...
            <div style={{ padding:'32px 32px 120px' }}>
              ...
              {(() => {
                const isNotABenefit = (s: string) =>
                  ...
                  /concentré\s+en\s+flacon|en\s+flacon\s+verre|100%\s+pure\s+et\s+naturelle|Usage\s+externe\s+uniquement|de\s+référence|essentielle\s+puissante|Savon\s+artisanal\s+Mille-Bulles|par\s+Les\s+Naturels\s+de\s+la\s+Source|Distillation\s+par\s+vapeur|sans\s+conservateur/i.test(s)
...
const FERME_SECTIONS: FermeSection[] = [
  ...
  { key:'laine', emoji:'🧶', name:'Laine & Créations', count:'Filature artisanale', boutiqueCat:'Laine',
